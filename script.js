...
function update() {
  ...
  if (reduceMotion.matches) {
    ...
    if (Math.abs(mouseX - targetMouseX) < 0.001) mouseX = targetMouseX;
    if (Math.abs(mouseY - targetMouseY) < 0.001) mouseY = targetMouseY;
  }
  ...
  const needsScrollUpdate = Math.abs(smoothScroll - targetScroll) > 0.08;
  const needsMouseUpdate = !reduceMotion.matches && (Math.abs(mouseX - targetMouseX) > 0.001 || Math.abs(mouseY - targetMouseY) > 0.001);
...
function setupSightSlider() {
  if (!sightsTrack || !originalCards.length) return;
...
function updateSightSlider() {
  ...
  const gap = parseFloat(window.getComputedStyle(sightsTrack).columnGap || window.getComputedStyle(sightsTrack).gap || "0") || 0;
...
document.querySelectorAll('.site-nav a, .site-logo').forEach((link) => {
  link.addEventListener('click', (e) => {
    ...
    if (href && href.startsWith('#')) {
      ...
      let targetPos = 0;
      if (targetHash === 'cinema') targetPos = 0;
      else if (targetHash === 'bridge') targetPos = 1100;
      else if (targetHash === 'bazaar') targetPos = 2140;
      else if (targetHash === 'routes') targetPos = 3500;
